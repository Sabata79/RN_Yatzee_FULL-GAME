...
export default function PlayerCardBGSelector() {
  ...
  // keep preferred in sync with level if DB hydrated a too-low choice
  useEffect(() => {
  ...
  const computeFanGeometry = (count, winWidth) => {
    const SIDE_PADDING = 2; // px, container horizontal padding
    ...
    const available = Math.max(MIN_AVAILABLE, winWidth - SIDE_PADDING * 2);
  ...
  // This ensures the fan block itself is centered precisely relative to the rest
  // of the UI instead of being stretched to the full window width.
  ...
  // Let flexbox size the fan container naturally and center it; avoid forcing width
  const fanContainerStyle = styles.fanContainer;
  ...
  if (loading) {
    // kevyt placeholder latauksen ajaksi; säilytetty minimissä
    return (
      ...
      <View style={styles.fanOuter}>
        <View style={fanContainerStyle}>
          {bgList.map((bg, idx) => {
            ...
            // Use a robust key so React ei ylikäytä väärää instanssia valinnan jälkeen
...
const styles = StyleSheet.create({
  ...
  fanItemWrap: {
    width: 90,
    alignItems: 'center',
    marginRight: 0, // overlap to create fan (dynamic overlap applied inline)
  },
  fanItem: {
    width: 90,
    height: 120,
  ...
  selected: {
    // visual highlight moved to overlay to avoid layout shift
    shadowColor: '#FFD600',
    shadowOpacity: 0.32,
    shadowRadius: 12,
    elevation: 10,
  ...
  // overlay used to show selected border without changing layout
  selectedOverlay: {
    position: 'absolute',
    top: 0,
    left: 0,
